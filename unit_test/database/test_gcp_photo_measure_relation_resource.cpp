...
TEST(TestGCPPhotoMeasureRelation, SimpleTest)
{
  using namespace hs::recon::db;
  typedef Database::Identifier Identifier;
  typedef Database::Float Float;

  typedef hs::test::ResourceTester<GCPPhotoMeasureRelationResource> Tester;
  ...
  ASSERT_EQ(Tester::TEST_SUCCESS,
            Tester::Test("test_gcp_photo_measure_relation_resource.db",
                         add_requests,
                         true_added_records));
}
